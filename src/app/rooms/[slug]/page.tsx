...
export default function RoomDetailPage() {
  ...
  if (error || !room) return (<><Header /><main className="flex-1 flex items-center justify-center min-h-screen"><div className="text-center"><h1 className="text-2xl mb-4">Room Not Found</h1><p className="text-muted mb-4">{error || "This room could not be found."}</p><Link href="/rooms" className="btn btn-primary">Browse Rooms</Link></div></main><Footer /></>);
  ...
  const img = room.room_images && room.room_images.length > 0 ? room.room_images[0] : null;

  return (<><Header /><main className="flex-1 pb-20 md:pb-0">
    <section className="relative h-[60vh] min-h-[400px] bg-cream">
      <div className="absolute inset-0">
        {img ? <img src={img.src} alt={img.alt || room.name} className="w-full h-full object-cover" /> : <RoomImage roomName={room.name} category={room.category} className="w-full h-full" />}
      </div>
    ...
    <section className="border-b border-border-light"><div className="container"><div className="room-facts">
      {[{ v: room.size_sqm + " m2", l: "Size" }, { v: "Up to " + room.max_guests, l: "Guests" }, { v: room.bed_type, l: "Bed" }, { v: room.view_type, l: "View" }, { v: room.floor_info, l: "Floor" }].map((f) => (<div key={f.l} className="text-center"><div className="room-fact-value">{f.v}</div><div className="room-fact-label">{f.l}</div></div>))}
    </div></div></section>
    <section className="section"><div className="container"><div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
      <div className="lg:col-span-2">
        ...
        {room.room_highlights && room.room_highlights.length > 0 && <div className="mb-10"><h3 className="text-xl mb-6">Room Highlights</h3><div className="grid grid-cols-2 gap-3">{room.room_highlights.map((h: any) => <div key={h.id} className="flex items-center gap-3 py-2"><Star className="w-4 h-4 text-gold flex-shrink-0" /><span className="text-charcoal-light">{h.text}</span></div>)}</div></div>}
        {room.amenity_groups && room.amenity_groups.length > 0 && <div className="mb-10"><h3 className="text-xl mb-6">Amenities</h3>{room.amenity_groups.map((g: any) => <div key={g.id} className="mb-6"><h4 className="text-sm font-medium uppercase tracking-wider text-muted mb-3">{g.group_name}</h4><div className="amenity-grid">{g.amenity_items.map((i: any) => <div key={i.id} className="amenity-item"><Check className="w-4 h-4 text-gold flex-shrink-0" /><span>{i.item_text}</span></div>)}</div></div>)}</div>}
